...
class analyser_recursive_look_ahead:
    ...
    def look_ahead_eval(self, fen, move: str, is_white_move: bool):
        self.stockfish.set_fen_position(fen, send_ucinewgame_token=self.send_uci_new_game)
    ...
    def get_first_depth_prob(self, starting_fen: str, move: str):
        self.stockfish.set_fen_position(starting_fen, send_ucinewgame_token=self.send_uci_new_game)
        for i in range(1, self.stockfish_depth + 1):
            self.stockfish.set_depth(i)
            best_move = self.stockfish.get_best_move()
            print(f"best: {best_move} {move} {i}")
            if best_move == move:
                return 0.7 * np.exp(-0.08 * i)

        self.stockfish.set_depth(self.stockfish_depth)
        return 0.7 * np.exp(-0.08 * (self.stockfish_depth + 1))
    ...
    def get_next_move(self, fen: str, parent: tree_node) -> Set[Tuple[str, tree_node]]:
        ...
        temp_set = set()
        self.stockfish.set_fen_position(fen, send_ucinewgame_token=self.send_uci_new_game)
        best_move_with_eval = self.stockfish.get_top_moves(1)
        ...
        if (len(best_move_with_eval) == 0):
            ...
            node = tree_node(evaluation, fen, [])
            parent.add_child(node)
            return set()

        best_move_with_eval = best_move_with_eval[0]
        top_moves = self.stockfish.get_top_moves(self.num_variation + 2)
